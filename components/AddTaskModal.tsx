...
const AddTaskModal: React.FC<AddTaskModalProps> = ({ onClose, onAdd, initialDate }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Converte a data selecionada para YYYY-MM-DD local
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in duration-300 border border-white/20">
        <div className="px-6 py-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
          <h2 className="text-xl font-black text-slate-800 tracking-tight">CRIAR TAREFA</h2>
            ...
            {isCalendarOpen && (
              <div className="absolute bottom-full mb-3 left-0 w-72 bg-white rounded-3xl shadow-2xl border border-indigo-100 p-5 animate-in slide-in-from-bottom-2 duration-300 z-[60]">
                ...
                <div className="grid grid-cols-7 gap-1 mb-2">
                  {['D', 'S', 'T', 'Q', 'Q', 'S', 'S'].map(d => (
                    <div key={d} className="h-8 w-8 flex items-center justify-center text-[9px] font-black text-slate-300">{d}</div>
                  ))}
                </div>
